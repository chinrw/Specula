...
- The final per-bug status is one of: REPRODUCED / ENV_LIMITED / MASKED / FALSE POSITIVE / NEEDS MORE INFO. `MASKED` is a **finding** (real defect, consequence masked today) — surfaced, not a false positive. A finding handed back to repair is recorded as **PENDING REPAIR (RR-NNN)**; its later lifecycle follows `references/repair-request-format.md`. If the repair loop is exhausted, the orchestrator files the request under `deferred/` and marks the finding `DEFERRED` — that is not your verdict to write.
